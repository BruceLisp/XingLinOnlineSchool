...
public class ConverVideoUtils {
	...
	private long currentTimeMills;			//时间戳
	private String videofolder = "D:\\privateData\\video\\other\\"; 		// 别的格式视频的目录
	private String targetfolder = "D:\\privateData\\video\\new\\"; 	// mp4视频的目录
	private String ffmpegpath = "D:\\privateData\\video\\utils\\ffmpeg-20171225-be2da4c-win64-static\\bin\\ffmpeg.exe";		 // ffmpeg.exe的目录
	private String mencoderpath = "D:\\privateData\\video\\utils\\mencoder.exe"; 	// mencoder的目录
	private String imageRealPath = "D:\\privateData\\video\\imgs\\";   // 截图的存放目录
	
	public ConverVideoUtils() {
	}

	public String getVideoUrl(){				//用来传输工具生成的图片和视频的地址
		return targetfolder+Long.toString(this.currentTimeMills)+".mp4";
	}

	public String getFilerealname(){				//用来传输工具生成的图片和视频的地址
		return this.filerealname;
	}

	public String getImageUrl(){				//用来传输工具生成的图片和视频的地址
		return imageRealPath+Long.toString(this.currentTimeMills)+".jpg";
	}
	...
	public boolean beginConver(String targetExtension, boolean isDelSourseFile) {
		...
		if (process(targetExtension,isDelSourseFile)) {
			...
			String temppath=videofolder + currentTimeMills + ".avi";
	...
	public boolean processImg(String sourceVideoPath) {
		...
		//检查转码后的视频是否存在，不存在返回错误
		if(!checkfile(sourceVideoPath)){
			System.out.println("视频不存在，截图失败！");
			return false;
		}
		...
		//第一帧： 00:00:10
		//截图命令：time ffmpeg -ss 00:00:10 -i test1.flv -f image2 -y test1.jpg
		
		commend.add(ffmpegpath);			//指定ffmpeg工具的路径
		commend.add("-ss");
		commend.add("00:00:10");			//10是代表第10秒的时候截图
		commend.add("-i");
		commend.add(sourceVideoPath);		//截图的视频路径
		commend.add("-f");
		commend.add("image2");
		commend.add("-y");
		commend.add(imageRealPath + currentTimeMills + ".jpg");		//生成截图xxx.jpg
	...
	private String processAVI(int type) {
		...
		commend.add(videofolder + currentTimeMills + ".avi");   //存放路径+名称，生成.avi视频
     ...
     // -xvidencopts bitrate=600 -of avi -o rmvb.avi
		...
		try {
          ...
          //启动两个线程，一个线程负责读标准输出流，另一个负责读标准错误流  
			...
			return videofolder + currentTimeMills + ".avi";		//返回转为AVI以后的视频地址
	...
	private boolean processVideoFormat(String oldfilepath, String targetExtension, boolean isDelSourceFile) {
		...
		commend.add("-strict");
		commend.add("-2");
		commend.add(targetfolder + currentTimeMills + targetExtension);  // //转码后的路径+名称，是指定后缀的视频
		...
		try {
			...
			System.out.println("生成mp4视频为:"+videofolder + currentTimeMills + ".mp4");
			return true;
			} 
			catch (Exception e) {
			e.printStackTrace();
